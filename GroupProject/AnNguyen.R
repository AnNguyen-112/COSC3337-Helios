...
#YOU SHOULD USE THIS ONLY WHEN THE DATA SET YOU IMPORTED DOESNT HAVE PROPER COLUMN NAMES.
#data2 = Solar_flare_RHESSI_2004_05
...
custom_color_palette <- c("#ffd92f", "#fc8d62", "#e78ac3", "#a6d854", "#66c2a5")
heatmap_batch1 <- ggplot(batch1_Map, aes(x = x.pos.asec, y = y.pos.asec, fill = intensity_method2,color = factor(energy.kev))) + 
  scale_color_manual(values = custom_color_palette) + 
  scale_fill_viridis_c() + 
  geom_tile(width = 10, height = 10) + 
  theme_minimal() + 
  labs(title = "Intensity Map: Months 1+2+3+4(year 2004) (Method 2)", x = "X Position", y = "Y Position")
heatmap_batch2 <- ggplot(batch2_Map, aes(x = x.pos.asec, y = y.pos.asec, fill = intensity_method2, color = factor(energy.kev))) + 
  scale_color_manual(values = custom_color_palette) + 
  scale_fill_viridis_c() + 
  geom_tile(width = 10, height = 10) + 
  theme_minimal() + 
  labs(title = "Intensity Map: Months 9+10+11+12(year 2005) (Method 2)", x = "X Position", y = "Y Position")
#Printing plots
